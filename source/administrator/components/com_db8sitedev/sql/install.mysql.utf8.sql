CREATE TABLE IF NOT EXISTS `#__db8sitedev_checks` (
...
SELECT * FROM ( SELECT 'Check','com_db8sitedev.check','{"special":{"dbtable":"#__db8sitedev_checks",
	"key":"id","type":"Check","prefix":"db8 Site DevTable"}}',
	'{"formFile":"administrator\/components\/com_db8sitedev\/models\/forms\/check.xml",
	"hideFields":["checked_out","checked_out_time","params","language" ,"description"],
...
SELECT * FROM ( SELECT 'Check Category','com_db8sitedev.category','{"special":{"dbtable":"#__categories","key":"id",
	...
	"core_catid":"parent_id", "core_xreference":"null", "asset_id":"asset_id"},
	...
	"targetTable":"#__categories","targetColumn":"id","displayColumn":"title"}]}') AS tmp
WHERE NOT EXISTS (
	SELECT type_alias FROM `#__content_types` WHERE (`type_alias` = 'com_db8sitedev.check')
) LIMIT 1;
